...
import com.tbohne.sqlite.processor.models.CreateProjectionModel;
...
public class ProjectionProcessor {
	...
	ProjectionProcessor(ProcessingEnvironment processingEnv) {
	...
	public void process(TypeElement projectionElement) {
		String packageName = processingEnv.getElementUtils().getPackageOf(projectionElement).getQualifiedName().toString();

		AnnotationSpec
				generated =
				AnnotationSpec.builder(Generated.class)
											.addMember("value", "\"com.tbohne.sqlite.processor.typedSqlProcessor\"")
											.addMember("comments", "\"from " + projectionElement.getQualifiedName() + "\"")
											//.addMember("date",  "\"" + Instant.now().toString() + "\"")
											.build();
		TypeSpec.Builder
				classBuilder =
				TypeSpec.classBuilder(projectionElement.getSimpleName() + "Sql")
								.addModifiers(Modifier.PUBLIC, Modifier.FINAL)
								.addAnnotation(generated)
								.addJavadoc(CodeBlock.builder().add("Generated from {@link $T}", projectionElement.asType()).build());
		...
		processIntoClass(projectionElement, classBuilder, pathToTableCache);

		try {
			JavaFile.builder(packageName, classBuilder.build()).build().writeTo(filer);
		} catch (IOException e) {
			messager.error(projectionElement, e.toString());
		}
	}

	public void processIntoClass(
			TypeElement projectionElement, TypeSpec.Builder classBuilder, HashMap<String, CreateTableModel> pathToTableCache)
	{
		CreateProjection createProjection = projectionElement.getAnnotation(CreateProjection.class);
		CreateProjectionModel
				model =
				CreateProjectionModel.build(processingEnv, messager, projectionElement, createProjection, pathToTableCache);
		addRowClass(classBuilder, projectionElement, model);
		addJavaCursorClass(classBuilder, projectionElement, model);
		addSqlCursorClass(classBuilder, projectionElement, model);
	}

	void addRowClass(TypeSpec.Builder classBuilder, TypeElement projectionElement, CreateProjectionModel model) {
		TypeSpec.Builder
				cursorClassBuilder =
				TypeSpec.classBuilder(model.rowName)
								.addModifiers(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
								.addJavadoc(CodeBlock.builder()
																		 .add("A query row for projection {@link $T}", projectionElement.asType())
																		 .build());
		MethodSpec.Builder constructorBuilder = MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC);

		addRowColumns(projectionElement, cursorClassBuilder, constructorBuilder, model);
			...
			CreateProjectionModel model)
	{
		...
		for (int i = 0; i < binders.size(); i++) {
			...
			if (types.isSameType(voidMirror, binder.fromSqlJavaMirror)) {
				...
				//TODO NULLABLE
				classBuilder.addField(FieldSpec.builder(javaTypeName, binder.columnJavaName, Modifier.FINAL, Modifier.PRIVATE)
																			 .build());
				...
				classBuilder.addMethod(MethodSpec.methodBuilder("get" + StringHelpers.sqlToJavaSuffix(binder.columnJavaName))
																				 .returns(TypeName.get(binder.fromSqlJavaMirror))
																				 .addStatement("return $N", binder.columnJavaName)
																				 .build());
			} else { //unknown java type :(
				//TODO NULLABLE
				classBuilder.addField(FieldSpec.builder(objectTypeName, binder.columnJavaName, Modifier.FINAL, Modifier.PRIVATE)
																			 .build());
				constructor.addParameter(objectTypeName, binder.columnJavaName);
				constructor.addStatement("this.$N = $N", binder.columnJavaName, binder.columnJavaName);
				classBuilder.addMethod(MethodSpec.methodBuilder(StringHelpers.sqlToJava("get" + StringHelpers.sqlToJavaSuffix(
																						 binder.columnJavaName) + "AsLong"))
																				 .returns(TypeName.LONG)
																				 .addStatement("return (Long) $N", binder.columnJavaName)
																				 .build());
				classBuilder.addMethod(MethodSpec.methodBuilder(StringHelpers.sqlToJava("get" + StringHelpers.sqlToJavaSuffix(
																						 binder.columnJavaName) + "AsDouble"))
																				 .returns(TypeName.DOUBLE)
																				 .addStatement("return (Double) $N", binder.columnJavaName)
																				 .build());
				classBuilder.addMethod(MethodSpec.methodBuilder(StringHelpers.sqlToJava("get" + StringHelpers.sqlToJavaSuffix(
																						 binder.columnJavaName) + "AsString"))
																				 .returns(stringClassName)
																				 .addStatement("return (String) $N", binder.columnJavaName)
																				 .build());
				classBuilder.addMethod(MethodSpec.methodBuilder(StringHelpers.sqlToJava("get" + StringHelpers.sqlToJavaSuffix(
																						 binder.columnJavaName) + "AsByte[]"))
																				 .returns(byteArrayName)
																				 .addStatement("return (Byte[]) $N", binder.columnJavaName)
																				 .build());
				classBuilder.addMethod(MethodSpec.methodBuilder(StringHelpers.sqlToJava("get" + StringHelpers.sqlToJavaSuffix(
																						 binder.columnJavaName) + "AsObject"))
	...
	void addSqlCursorClass(TypeSpec.Builder classBuilder, TypeElement projectionElement, CreateProjectionModel model) {
		TypeSpec.Builder
				cursorClassBuilder =
				TypeSpec.classBuilder(model.sqlCursorName)
								.addModifiers(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
								.addJavadoc(CodeBlock.builder()
																		 .add("A cursor exposing the raw sql columns from projection {@link $T}",
																					projectionElement.asType())
																		 .build());
	...
	void addSqlCursorColumns(TypeSpec.Builder classBuilder, CreateProjectionModel model) {
		...
		for (int i = 0; i < columns.size(); i++) {
			...
			classBuilder.addMethod(MethodSpec.methodBuilder(doubleMethodName)
			...
			classBuilder.addMethod(MethodSpec.methodBuilder(stringMethodName)
			...
			classBuilder.addMethod(MethodSpec.methodBuilder(blobMethodName)
	...
	void addJavaCursorClass(TypeSpec.Builder classBuilder, TypeElement projectionElement, CreateProjectionModel model) {
		TypeSpec.Builder
				cursorClassBuilder =
				TypeSpec.classBuilder(model.javaCursorName)
								.addModifiers(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
								.addJavadoc(CodeBlock.builder()
																		 .add("A cursor exposing Java type-safe columns from projection {@link $T}",
																					projectionElement.asType())
																		 .build());
		cursorClassBuilder.addMethod(MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC).build());
		addCursorWrapperMethods(cursorClassBuilder);
	...
	void addBinderSet(TypeSpec.Builder classBuilder, CreateProjectionModel model) {
		//TODO
	}
}
