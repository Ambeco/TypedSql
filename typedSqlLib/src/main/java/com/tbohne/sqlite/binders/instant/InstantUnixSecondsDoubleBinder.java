 ...
 **/
public class InstantUnixSecondsDoubleBinder
		implements SimpleColumnBinder<@Nullable Instant, @Nullable Double>
{
	@Override
	public @Nullable Instant fromSql(@Nullable Double columnValue) {
		if (columnValue == null) {
			return null;
		}
		long seconds = (long) (double) columnValue;
		long nanos = (long) ((columnValue - seconds)*1000000000);
		return Instant.ofEpochSecond(seconds, nanos);
	}

	@Override
	public @Nullable Double toSql(@Nullable Instant javaValue) {
		if (javaValue == null) {
			return null;
		}
		return javaValue.getEpochSecond() + javaValue.toEpochMilli()/1000000000.0;
	}
}
